...
class Extractor(object):
    ...
    def _io_err(function, path, excinfo):
        ...
        """
        print(f"!! {function}: Cannot delete {path}!\n{excinfo}")
...
class ExtractionItem(object):
    ...
    def extract(self):
        ...
        if self.depth > ExtractionItem.RECURSION_DEPTH:
            self.printf(f">> Skipping: recursion depth {self.depth:d}")
            return self.get_status()

        # check if checksum is in visited set
        self.printf(f">> MD5: {self.checksum}")
        with Extractor.visited_lock:
            ...
            if (self.checksum in self.extractor.visited and
                ...
                self.printf(f">> Skipping: {self.checksum}...")
                return self.get_status()
            else:
                self.extractor.visited[self.checksum] = self.status
        ...
        try:
            ...
            self.printf(
                f">> Status: Kernel: {self.get_kernel_status()}, "
                f"Rootfs: {self.get_rootfs_status()}, "
                f"Do_Kernel: {self.extractor.do_kernel}, "
                f"Do_Rootfs: {self.extractor.do_rootfs}")
            ...
            for module in binwalk.scan(self.item, "-e", "-r", "-C", self.temp,
                ...
                for entry in module.results:
                    ...
                    self.printf(f"========== Depth: {self.depth:d} ===============")
                    self.printf(f"Name: {self.item}")
                    self.printf(f"Desc: {desc}")
                    self.printf(f"Directory: {dir_name}")
...
#        print (filetype)
        if filetype:
            if any(s in filetype for s in ["application/x-executable",
                                           ...
                                           "application/msword",
                                           "image/", "text/", "video/"]):
                self.printf(f">> Skipping: {filetype}...")
                return True
...
#        print ('-=----------------------------')
        ...
        for black in black_lists:
            if self.item.endswith(black):
                self.printf(f">> Skipping: {self.item}...")
                return True
    ...
    def _check_firmware(self, module, entry):
        ...
        if 'header' in desc:
            ...
            if "uImage header" in desc:
                ...
                #     self.printf(f">>>> {entry.description}")
            ...
            # TP-Link or TRX
                ...
                self.printf(f"image size: {image_size:d}")
                self.printf(f"rootfs offset: {rootfs_offset:d}")
                self.printf(f"rootfs size: {rootfs_size:d}")
                self.printf(f"kernel offset: {kernel_offset:d}")
                self.printf(f"kernel size: {kernel_size:d}")
        ...
        if 'filesystem' in desc or 'archive' in desc or 'compressed' in desc:
            if self.get_rootfs_status(): return True
                ...
                if dir_name:
                    ...
                    if self.output:
                        # changed this from the original to write the files instead of archiving them
                        # only write when this exists
                        if not os.path.exists(self.output):
                            shutil.copytree(unix[1], self.output, symlinks=True)
                    else:
    ...
    def _check_recursive(self, module, entry):
        ...
        if 'filesystem' in desc or 'archive' in desc or 'compressed' in desc:
            if dir_name:
                self.printf(f">> Recursing into {desc} ...")
                count = 0
