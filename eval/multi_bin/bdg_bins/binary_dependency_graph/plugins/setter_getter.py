...
M_SET_KEYWORD = ('set', 'insert')
M_GET_KEYWORD = ('get', 'read')
LIB_KEYWORD = 'lib'


class SetterGetter(Plugin):

    def __init__(self, *kargs, **kwargs):
        Plugin.__init__(self, 'setter_getter', *kargs, **kwargs)
    ...
    def run(self, key_string, key_addr, reg_name, core_taint, current_path):
        ...
        if node and node.name and '+' not in node.name:
            ...
            if are_parameters_in_registers(p):
                ...
                print "setter_getter.run: Implement me"
                return

            if (self.is_setter(str(node.name)) or self.is_getter(str(node.name))) and tainted:
                ...
                # set_env
                role = Role.GETTER if self.is_getter(str(node.name)) else Role.SETTER
    ...
    def discover_new_binaries(self):
        ...
        for role, string, name_fun in zip(self._roles, self._strings, self._name_funs):
            ...
            if role == Role.SETTER:
                ...
                for set_f in M_SET_KEYWORD:
                    for get_f in M_GET_KEYWORD:
                        dual_fun = self.case_sensitive_replace(name_fun, set_f, get_f)
