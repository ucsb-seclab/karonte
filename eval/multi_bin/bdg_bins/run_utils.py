...
def get_args_call(p, no):
    ...
    ins_args = get_ord_arguments_call_callee(p, no.addr)
    if not ins_args:
...
def get_initial_state(p, starting_addr, ct, initial_taint=None):
    """
    ...
    for taint_addr in initial_taint:
        size = ct._taint_buf_size
        t = ct._get_sym_val(name=ct._taint_buf + '_InitTaint_', bits=size).reversed
        s.memory.store(taint_addr, t)
        ct._taint_applied = True
