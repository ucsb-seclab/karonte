# HELP
# Analyze each binary in a firmware assuming that attacker-controlled data comes from any IPC channel.
import angr
...
class AlertTest:
    ...
    def get_bins(self):
        ...
        pf.run(pickle_file=self.fw_pickle)
        pf_str = pf.get_strs(end=3)
        bins = pf.get_bins()
        filtered_bins = []
        for b in bins:
            new_b = b
        ...
        bdg.run()
        return [n.bin for n in bdg.nodes]

    def get_additional_taint(self, p, f_addr, sink_addr):
        if 'ARM' not in self.p.arch.name:
            raise ("Sorry this test is only meant for ARM binaries :(")
    ...
    def run(self):
        ...
        dirs = RESULT_DRECTORY + '/' + parent_dir

        if not os.path.exists(dirs):
            os.makedirs(dirs)
            
        sum_res = open(dirs + '/summary_result.log', 'w')
        self.ignore += os.listdir(RESULT_DRECTORY + '/' + parent_dir + '/')
        ...
        for b in list(set(bins)):
            try:
                ...
                self.fp = open(RESULT_DRECTORY + '/' + parent_dir + '/' + bin_name, 'w')
