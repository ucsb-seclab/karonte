...
class AlertTest:
    ...
    def get_bins(self):
        p = sp.Popen('find '+ self.fw_directory + ' -executable -type f', stdout=sp.PIPE, stderr=sp.PIPE, shell=True)
        o, e = p.communicate()
        return o.split('\n')
    ...
    def run(self):
        ...
        for b in list(set(bins)):
            try:
                ...
                if 'ARM' not in self.p.arch.name:
                    print "Sorry this test is only thought for ARM binaries :("
                    break
        ...
        sum_res.close()
        term_ana_name = self.fw_directory.replace('.', '').replace('/', '_')
        term_ana = open(self.result_dir + '/{}_finished.res'.format(term_ana_name), 'w')
        term_ana.write("Analysis Terminated.\n")
        term_ana.close()
        print 'Done.'
